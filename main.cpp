...
int main() {
    ...
    ClubManager<Club> c;
    while (1) {
        cout << "Enter\n1. Add club\n2. Modify club\n3. View club\n4. Delete club\n5. Top n clubs\n6. View all Clubs\n7. Save Clubs to File\n8. Load Clubs from File" << endl;
        cout << "Enter your choice:";
        cin >> ch;


        switch (ch) {
            case 1: c.create_club(); break;
            case 2: c.modify_club(); break;
            case 3: c.view_club(); break;
            case 4: c.delete_club(); break;
            case 5:
                ...
                c.top_clubs(n);
                break;
            case 6: c.view_all(); break;
            case 7: {
    ofstream outFile(filename);
    if (!outFile) {
        cerr << "Error opening file for saving.\n";
        break; // Corrected: use 'break' instead of 'continue'
    }
    ...
    for (const auto& category : c.clubs) {
        outFile << "Category: " << category.first << endl;
        outFile << "Number of Clubs: " << category.second.size() << endl << endl;


        for (const auto& clubEntry : category.second) {
            Club* club = clubEntry.second;
            club->save(outFile);
            outFile << endl;  // Separate clubs with a blank line
        }
    }


    outFile.close();
    cout << "Clubs saved to file successfully.\n";
    break;
}


case 8: {
    ifstream inFile("clubs.txt");
    if (inFile.is_open()) {
        string line;
        string category;
        while (getline(inFile, line)) {
            if (line.find("Category:") == 0) {
                category = line.substr(10); // Extract category name
                cout << "Read category: " << category << endl;
            } else if (line.find("Club Name:") == 0) {
                string clubType;
                if (!getline(inFile, clubType)) {
                    cout << "Failed to read club type for club.\n";
                    break;
                }
                clubType = clubType.substr(0, clubType.find(" "));
             
                Club* club = nullptr;
                if (clubType == "Cult") {
                    club = new Cult();
                } else if (clubType == "General") {
                    club = new General();
                } else if (clubType == "Competitive") {
                    club = new Competitive();
                } else {
                    cout << "Unknown club type: " << clubType << endl;
                    continue; // Skip this club
                }


                if (club != nullptr) {
                    cout << "Created club object for category: " << category << " with name: " << club->name << endl;
                    club->load(inFile);  // Load the club data
                    c.clubs[category][club->name] = club; // Store the club in the map
                    cout << "Successfully loaded club: " << club->name << endl;
                }
            }
        }
        inFile.close();
        cout << "Clubs loaded from file successfully.\n";
    } else {
    ...
    break;
        ...
        cout << "Do you want to continue performing Club operations? 1 for yes and 0 for no" << endl;
        cin >> ch;
        if (ch !=1) break;
    }
...
case 8:{
loadClubsFromFile("clubs.txt", clubs);
    ...
    if (inFile.is_open()) {
        ...
        //while (getline(inFile, line)) {
            if (line.find("Category:") == 0) {
                ...
                clubType = clubType.substr(0, clubType.find(" "));
                cout<<"club type substr: "<<clubType<<endl;
                Club* club = nullptr;
                if (clubType == "Cult") {
                    club = new Cult();
                } else if (clubType == "General") {
                ...
                if (club != nullptr) {
                    cout << "Created club object for category: " << category << " with name: " << club->name << endl;
                    club->load(inFile);  // Load the club data
                    clubs[category][club->name] = club; // Store the club in the map
                    cout << "Successfully loaded club: " << club->name << endl;
                }
            }
        }
